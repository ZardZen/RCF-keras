...
def args_parse():
    ...
    ap.add_argument("-dtrain", "--data_path", default="imgs/train/data", required=False,
                    help="path to input image")
    ap.add_argument("-dlabel", "--label_path", default="imgs/train/label", required=False,
...
def create_train_data(data_path, img_type, rows, cols, label_path, npy_path):
    ...
    for imgname in imgs:
        ...
        i += 1
    np.save(npy_path + 'data_train.npy', imgdatas)
    np.save(npy_path + 'label_train.npy', imglabels)
...
def create_test_data(test_path, img_type, rows, cols, tlabel_path, npy_path):
    ...
    print(len(imgs))
    imgdatas = np.ndarray((len(imgs), rows, cols,3), dtype=np.uint8)
    imgtlabels = np.ndarray((len(imgs), rows, cols, 1), dtype=np.uint8)
    for imgname in imgs:
        midname = imgname[imgname.rindex("/")+1:]
        img = load_img(test_path + "/" + midname)
        img = img_to_array(img)
        label = load_img(tlabel_path + "/" + midname, grayscale = True)
        label = img_to_array(label)
        imgdatas[i] = img
        imgtlabels[i]= label
        i += 1
